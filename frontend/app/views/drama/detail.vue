...
async function loadSourceVersions() {
  ...
  try {
    const result = await dramaAPI.sourceVersions(dramaId)
...
async function inspectSourceHealth() {
  ...
  try {
    sourceHealth.value = await dramaAPI.sourceHealthCheck(dramaId)
  } catch (e) {
...
async function startSourceCleanup() {
  ...
  sourceCleanupError.value = ''
  try {
    sourceCleanupTask.value = await dramaAPI.startSourceCleanup(dramaId)
...
.source-version-row strong { display: block; color: var(--text-1); font-size: 11px; }.source-version-row small { display: block; margin-top: 2px; color: var(--text-3); font-family: var(--font-mono); font-size: 9px; }
.source-current-badge, .source-candidate-badge { padding: 3px 7px; border-radius: 99px; font-size: 9px; font-weight: 700; }.source-current-badge { background: var(--success-bg); color: var(--success); }.source-candidate-badge { background: color-mix(in srgb, var(--warning) 16%, var(--surface-raised)); color: var(--warning); }
