...
| HB-20260912-09 | Issue #135：production-package-preview 的 nonce 并发用例在全量测试下随机失败（flaky） | 已合入并关闭（PR #140，merge `b5177c0`，2026-09-12T09:03:42Z；首轮 Request changes 1 需修 + 3 建议 → 逐条处理 → 复查 APPROVED）。**根因**：原生文件锁把 `open(wx)` 失败一律当作「已被占用 → 等到超时」，而 Windows 下 `unlink` 后目录项短暂可见、并发 create 返回 `EPERM` / `EACCES` / `EBUSY` 都不属于「竞争」，故仅在并发噪声足够大（同文件其他用例的 300ms 级占用）时才稳定命中，表现为单跑通过、全量随机失败。**修复**：① `EPERM` / `EACCES` / `EBUSY` 纳入可重试集合；② 等待上限 `30s → 10s`（对齐 MySQL `GET_LOCK(?, 10)` 与请求体预算），新增 `PREVIEW_AUTH_NONCE_LOCK_TIMEOUT_MS` 可注入；③ 固定 `sleep(5)` 改为递增退避 5ms→50ms；④ 超时前 `console.warn` 输出 `lockPath` / 等待时长 / 尝试次数 / 最后错误码；⑤ 超时仍抛 `PREVIEW_NONCE_STORE_UNAVAILABLE`，保持 fail-closed 不降级放行。**复核意见**：需修 1 条（上限与注释所称「对齐 MySQL 10s」矛盾）、建议 3 条（超时分支无测试、长等待缺可观测性且注释论证不成立、重试密度），其中复核指出 `fsp.mkdir(recursive)` 对已存在目录不校验写权限、故「mkdir 成功即可排除权限问题」的论证不成立 —— 已核实属实并删除该论证。**验证**：typecheck 通过；合入后 `master@b5177c0` 定向连续 3 轮各 **29 / 29 pass / 0 fail**（flaky 消除的直接证据，含新增「注入 40ms 上限 + 占位锁 → 断言 `PREVIEW_NONCE_STORE_UNAVAILABLE`」用例）、全量 **325 / 325 pass / 0 fail / 0 skipped**（修复前 323）；PR CI 两轮 backend + frontend 全 success。合并时 approve 后 `mergeable_state` 仍为 `blocked`（checks 全绿 + APPROVED 均已满足，疑为门禁状态未刷新），改用 `--admin` squash 合入。**收口**：`#135` 发布交付说明（根因 / 修复 / 验证 / 复核采纳对照）并以 completed 关闭，远端与本地分支均已清理。#135 是 #121 批次 B-1 与 #130 复验中唯一反复出现的失败项，本轮后 `master` 全量稳定 | [查看 PR #140](https://github.com/Aibrother258/JisuVideo-ai/pull/140) |
