...
| HB-20260912-05 | Issue #130：前端体验与文档小项批次（P2-1 / P2-2 / P2-9 / P2-12 / P3-1 / P3-2 / P3-3） | 实施完成待评审（主仓库基线 `541cbef9`，实施线 `production-reliability`）：README 视频能力宣称与实现对齐（字幕/配音标注未实现）、`source_versions` 懒生成补文档、`defaultEpisodeCount` 除数 3500→1000（2635 字样本由 1 集改为 3 集）、每集标题「第N集：」前缀剥离（纯函数 + 11 组边界单测）、集数输入框常驻并带同口径起步值、导入区 Markdown 丢失提示；P2-12 经核实为「当前代码上不成立」（缓存 10 秒 TTL + 三个写入口均已失效），改为新增结构回归测试锁定不变量。本地验证：`npm run typecheck` 通过、backend **300 pass / 0 fail / 0 skipped**、frontend **161 pass / 0 fail / 0 skipped**、`npm run build` 通过；浏览器级手工复核未做，P2-9/P2-1 未做真实模型端到端复现 | [查看日志](./2026-09-12-issue130-frontend-docs-batch.md) |
