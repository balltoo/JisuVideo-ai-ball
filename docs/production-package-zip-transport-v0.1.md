...
- 拒绝以 `/`、`\\` 或盘符（如 `C:`）开头的绝对路径；
- 拒绝包含空字节的名称；
- 条目名允许使用反斜杠作为**相对路径分隔符**（Windows 自带压缩工具"压缩到 ZIP" / `Compress-Archive` 的默认行为）；实现必须先规范化为 POSIX `/` 再执行本节全部校验，该规范化不得放宽任何拒绝条件（Issue #120）；
- 拒绝任意 `..` 路径段；
...
生产环境的快照不能只保存在某个 Node 进程的内存中：`preview_package_snapshots` MySQL
表是 token、所属身份、指纹、过期时间和预览 DTO 的唯一索引来源；原始 ZIP 与解压包位于
`PREVIEW_SNAPSHOT_ROOT` 的共享持久化存储。每个后端副本必须同时连接同一个 MySQL 和同一份
可读写快照存储，才能保证“副本 A 上传，副本 B 读取”成立。当前 Compose 将后者挂载为
`/app/data/production-package-previews`。若未来跨主机横向扩容，必须改用 RWX 卷或对象存储，
不得把各主机各自的本地磁盘误当成共享存储。
...
- 过期清理先删除 MySQL 中的共享索引，再幂等删除对应快照目录；任意副本执行该任务都不得
  让已删除的 token 被另一副本的内存缓存或遗留 `snapshot.json` 重新恢复；
...
- [ ] `../x`、绝对路径（含以反斜杠开头的绝对路径）、盘符路径和 NUL 文件名；
- [ ] 反斜杠作为相对路径分隔符（Windows 自带压缩工具生成）被规范化为 `/` 后正常接受（Issue #120）；
...
- [ ] token 不能跨用户使用；
- [ ] 两个独立后端进程中，A 上传的 token 可由 B 在同一身份下读取；不同身份仍返回 404；
